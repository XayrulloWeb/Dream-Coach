...
function fitLabel(player: SimulationPlayer): { text: string; color: string } {
  ...
  if (role === natural || preferred.includes(role)) {
    return { text: 'Perfect Fit', color: 'text-emerald-300' };
  }

  if (roleBand(role) === roleBand(natural)) {
    return { text: 'Playable', color: 'text-amber-300' };
  }

  return { text: 'Poor Fit', color: 'text-rose-300' };
}
  ...
  if (!active) {
    return (
      <div className="min-h-screen bg-[#050A15] text-white px-4 py-8 pb-24">
        <h1 className="font-['Lexend'] text-2xl">Player Profile</h1>
        <p className="mt-3 text-slate-400">No squad found. Build squad first.</p>
        <button
    ...
    <div className="min-h-screen bg-[#050A15] text-white pb-24">
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-4 pt-6 space-y-4">
        <header className="flex items-center justify-between">
          ...
          <h1 className="font-['Lexend'] text-xl text-emerald-300 tracking-wide">PLAYER PROFILE</h1>
          <div className="w-8" />
        </header>
        ...
        <section className="rounded-2xl border border-white/10 bg-[#08162B]/90 p-4">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
            <Meta label="Natural" value={active.naturalPosition} />
            <Meta label="Role" value={active.rolePosition} />
            <Meta label="Stamina" value={`${active.stamina}%`} />
            <Meta label="Work Rate" value={`${active.attackWorkRate}/${active.defenseWorkRate}`} />
          </div>
        </section>

        <section className="rounded-xl border border-white/10 bg-[#08162B]/90 p-4">
          <p className="text-xs uppercase tracking-[0.12em] text-slate-400">Squad Players</p>

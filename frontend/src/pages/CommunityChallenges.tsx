...
export default function CommunityChallenges() {
  ...
  const onStartChallenge = (challenge: Challenge) => {
    ...
    setMessage(`Challenge armed: ${challenge.title}`);
    navigate('/match-setup');
  };

  const onMarkCompleted = async (challenge: Challenge) => {
    try {
      await submitChallengeRun({
        id: challenge.id,
        status: 'COMPLETED',
        score: 100,
        payload: { source: 'community-page-quick-mark' },
      });
      setMessage(`Challenge completed: ${challenge.title}`);
    } catch (reason) {
  ...
  return (
    <AppShell title="Community" activeTab="community" hideHeader>
      <header className="w-full z-40 bg-[var(--color-surface)] border-b border-white/5 pt-safe sticky top-0">
        <div className="flex items-center px-5 py-4">
          ...
          <div>
            <h1 className="font-['Lexend'] text-lg text-white">Community</h1>
            <p className="text-[10px] uppercase tracking-wider text-[var(--color-warning)] font-bold">Daily Challenges</p>
          </div>
        </div>
      </header>
      ...
      <main className="px-5 py-6 space-y-6">
        ...
        <section className="relative overflow-hidden rounded-2xl border border-[var(--color-warning)]/30 bg-[var(--color-warning)]/10 p-5 shadow-[0_0_20px_rgba(245,158,11,0.05)]">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="font-['Lexend'] text-sm tracking-wide text-[var(--color-warning)] uppercase">Daily Challenge</h3>
            </div>
            <h2 className="mt-2 text-xl font-semibold text-white">{challengeOfDay?.title ?? 'No challenge today'}</h2>
            <p className="mt-2 text-sm text-[var(--color-on-surface-variant)]">{challengeOfDay?.description ?? 'Please check later.'}</p>
            {challengeOfDay && (
        ...
        <section className="glass-panel rounded-2xl p-5">
          <p className="text-xs uppercase tracking-widest text-[var(--color-on-surface-variant)] font-bold mb-4">Challenge Pool</p>
          {loading ? <p className="text-sm text-[var(--color-on-surface-variant)]">Loading challenges...</p> : null}
          {!loading && !items.length ? <p className="text-sm text-[var(--color-on-surface-variant)]">No challenges yet.</p> : null}
        ...
        <section className="glass-panel rounded-2xl p-5 mb-6">
          <p className="text-xs uppercase tracking-widest text-[var(--color-on-surface-variant)] font-bold mb-4">Leaderboard</p>
          <div className="space-y-2">
            {LEADERBOARD.map((row) => (
              <div key={row.rank} className="rounded-xl border border-white/5 bg-[var(--color-surface-container-high)] p-3 flex items-center justify-between">
                ...
                <p className="text-sm text-[var(--color-primary)] font-bold">{row.points} pts</p>
              </div>
            ))}
          </div>
        </section>
